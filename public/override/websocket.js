...
const types = [
    {
        type: `market.depth.${currency.symbol}.step0.level20`,
        handle(data) {
            {
                ...
                cLast.innerText = lastTrade.price;
                ...
                ['long', 'short'].forEach(clx => {
                    ...
                    elm.innerText = calcAvailableVolume(clx, lastTrade);
                })
...

            {
                ...
                const all = [];
                {
                    ...
                    asks.slice(-askShowLength).forEach((ask, i) => {
                ...
                } // ASKs
                {
                    ...
                    bids.slice(-bidShowLength).reverse().forEach((ask, i) => {
                        const item = wrapper.children[i];
                        const progress = item.querySelector(".progress");
                        progress.style.width = `${ask.percent}%`
                        const price = item.querySelector(".price");
                ...
                window.mids = Object.fromEntries(mids.map((v, i) => [types[i], v]));
                window.basisRate = Object.values(mids).reduce((t,v) => t+v, 0) / mids.length;
            } // Handle Order Book
        }
    },
    {
        type: `market.contracts.${currency.symbol}`,
        handle({data}) {
